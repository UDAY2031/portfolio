...
export class Maze {
    constructor(rooms, seed = 2031) {
        ...
        for (let i=0;i<this.nodes.length-1;i++) {
            ...
            this.transitions.push(roundedPath(points));
        }
    ...
    sample(s) {
        ...
        if (s.rt<4 && s.room>0) { transition=s.room-1; u=(s.rt+8)/12; }
        ...
        if(transition>=0) {
            ...
            const travel=cubic(u); position=curve.getPointAt(travel);
            ...
            q.multiply(new T.Quaternion().setFromAxisAngle(new T.Vector3(0,0,1),Math.sin(u*Math.PI)*.436*(transition%2?1:-1)));
        ...
        return { position, q, traveling, fold, transition, fov: mix(62, 75 + 13*clamp(Math.abs(s.velocity||0)*70),traveling) };
    }
    apply(camera,s,reduced=false) {
        ...
        if(!reduced) {
            camera.position.add(new T.Vector3(noise(s.t*.15,4),noise(s.t*.12,61),noise(s.t*.11,7)).multiplyScalar(.02*amplitude));
            camera.rotateZ(noise(s.t*.08,19)*.0052*amplitude);
        }
